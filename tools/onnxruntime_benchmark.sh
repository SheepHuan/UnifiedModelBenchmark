...
adb -s $adb_device push --sync release/ort_v1.15.1-arm_nnapi/libonnxruntime.so $workspace/libs
...
adb -s $adb_device shell "cd $workspace;
  ...
  ./data/local/tmp/hcp/ort_benchmark --graph=$workspace/models/$model_name \
  --backend=arm \
  --num_threads=4"